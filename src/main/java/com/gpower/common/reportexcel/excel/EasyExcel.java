...
public final class EasyExcel implements Closeable {
	...
	private Workbook createWorkbook() throws IOException, InvalidFormatException {
		...
		if (!file.exists()) {
			...
			// HSSF-->HSSFWorkbook、XSSF-->XSSFWorkbook、SXSSF-->SXSSFWorkbook
		} else {
		...
		try {
			...
			for (int i = 0, length = list.size(); i < length; i++) {
				...
				for (int j = 0; j < fieldSize; j++) {
					...
					String[] mapping = info.getMapping();
					Object object;
					// 如果存在el表达式，计算他
					...
					case TEXT:
						...
						Map map = convertToMap(mapping);
						if (map != null) {
							value = getValueFromMapping(value, map);
						}
	...
	private Map<String, String> convertToMap(String[] str) {
		...
		for (String s : str) {
			try {
				...
				throw new RuntimeException("mapping格式设置不正确",e);
			}
		}
	...
	private String getValueFromMapping(String key, Map<String, String> map) {
