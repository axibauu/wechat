...
public class KickoutSessionControlFilter extends AccessControlFilter {
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
        ...
        String username = subject.getPrincipal() + ".kickout";
        ...
        while (it.hasNext()) {
            ...
            } catch (UnknownSessionException e1) {
                deque.remove(sessionID);
            }
        }
        ...
        if (session.getAttribute("kickout") != null) {
            ...
            //String requestTime = ServletRequestUtils.get  StringParameter(request, "RequestTime");
            if (!StringUtils.isEmpty(requestTime)) {
                //session过期
                WebUtils.saveRequest(request);
                WebUtils.toHttp(response).sendError(ResultStatus.SESSION_TIMEOUT.status());
            } else {
