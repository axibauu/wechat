(function ($) {
    ...
    $.fn.bootstrapTreeTable = function (options, param) {
        ...
        target.renderRow = function (tr, item) {
            $.each(options.columns, function (index, column) {
                if (column.field == 'operation' && item.type == '0') {
                    var td = $('<td style="text-align:right;width:100px"></td>');
                    if (column.button) {
                        var _ipt = $('<span><input type="button" class="btn btn-primary small blue" onclick="insertBtn(' + item[options.id] + ')" value="新增"/></span>' +
                            '<span><input type="button" class="btn btn-primary small red" onclick="deleteBtn(' + item[options.id] + ')" value="删除"/></span>'
                        );
                        td.append(_ipt);
                    }
                    tr.append(td);
                } else if (column.field == 'operation' && item.type == '1') {
                    var td = $('<td style="text-align:right;width:100px"></td>');
                    if (column.button) {
                        var _ipt = $(
                            '<span><input type="button" class="btn btn-primary small red" onclick="deleteBtn(' + item[options.id] + ')" value="删除"/></span>');
    ...
    $.fn.bootstrapTreeTable.defaults = {
        ...
        expanderExpandedClass: 'glyphicon glyphicon-folder-open',// 展开的按钮的图标
        expanderCollapsedClass: 'glyphicon glyphicon-folder-close'// 缩起的按钮的图标
        // glyphicon-file 文件图标
